# Sunmi-Payment-Library

</br>

## 📂 Preparation
Before starting, make sure you have imported the 'sunmiPaymentLibrary.jar' lib into your project and very importantly you must copy the Callback 
folder and the Util folder (without them you will not be able to use the lib), I also recommend copying the InitActivity, BaseApp and MainActivity
classes, these already interact as necessary with the lib and can be modified according to the needs of your project.
...
```bash
    EmvUtil().init()
```
Note: Every country have different format.
...
Congregate this is the last step and nothing to do so. If the code is `0` mean you will be ok, otherwise error will be show on `p1`
